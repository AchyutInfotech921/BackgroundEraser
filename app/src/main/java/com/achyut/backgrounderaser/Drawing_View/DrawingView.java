...
import com.achyut.backgrounderaser.R;
import com.achyut.backgrounderaser.Drawing_View.brushes.BrushSettings;
import com.achyut.backgrounderaser.Drawing_View.brushes.Brushes;
...
public class DrawingView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        @SuppressLint("NewApi") int contentWidth = minDimension + getPaddingStart() + getPaddingEnd();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        System.out.println("mMoveMode " + mMoveMode + "");
        if(!mMoveMode) {
            if (mZoomMode) {
                System.out.println("in to if!!!!!!!!!");
                return handleZoomAndTransEvent(event);
            } else if (event.getPointerCount() > 1) {
                System.out.println("in to else!!!!!!!!!!!!!!!");
                mZoomMode = true;
                return handleZoomAndTransEvent(event);
            } else if (event.getPointerCount() == 1) {
                mZoomMode = false;
                exitZoomMode();
                System.out.println("goes in to event 111");
                float scaledX = (event.getX() - mDrawingTranslationX) / mScaleFactor;
                float scaledY = (event.getY() - mDrawingTranslationY) / mScaleFactor;
                event.setLocation(scaledX, scaledY);
                mDrawingPerformer.onTouch(event);
    ...
    public boolean handleZoomAndTransEvent(MotionEvent event) {
        super.onTouchEvent(event);
        System.out.println("handleZoomAndTransEvent");
        if(!mMoveMode) {
            System.out.println("movemode!!! "+mMoveMode+"");
            if (event.getActionMasked() == MotionEvent.ACTION_POINTER_UP && event.getPointerCount() == 1)
                return false;
            if (event.getPointerCount() > 1) {
                translateAction = false;
            ...
            if (event.getActionMasked() == MotionEvent.ACTION_UP) {
                translateAction = false; // reset
                System.out.println("zoom offfffffff");
                mZoomMode = false;
    ...
    /** Developed 02-oct-19 by pragna
     * updated on 09-oct-19 for change return datatype
     * updated on 23-nov-19 for null value validation
     *
    ...
    /** Developed 21-sep-19 by pragna
     * updated on 28-sep-19 for change return datatype
